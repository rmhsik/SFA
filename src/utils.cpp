...
void calc_vecpot(double *efield, double *t, const int nt, double *afield){
    ...
    double* calc_momentum3D(double *afield_x, double *afield_y, double* afield_z, const int idx_ion, const int idx_rec, double *t){
        double dt = t[1] - t[0];
        double canonical_momentum_x = 0.0;
        double canonical_momentum_y = 0.0;
        double canonical_momentum_z = 0.0;
        double *canonical_momentum = new double[3];
        double tau_ion = t[idx_ion];
        double tau_rec = t[idx_rec];
        ...
        canonical_momentum[0] = canonical_momentum_x*prefactor*dt;
        canonical_momentum[1] = canonical_momentum_y*prefactor*dt;
        canonical_momentum[2] = canonical_momentum_z*prefactor*dt;
        return canonical_momentum;
    }
    ...
    cdouble* calc_matrix_element3D(double *ps, double afield_x, double afield_y, double afield_z, double Z, double n, double E0, double Ip){
        double a0 = 1.0; double q0 = 1.0;
        double ps_mod = sqrt(  (ps[0]+afield_x)*(ps[0]+afield_x) 
                             + (ps[1]+afield_y)*(ps[1]+afield_y) 
                             + (ps[2]+afield_z)*(ps[2]+afield_z));
        cdouble *tmp_value = new cdouble [3];
        tmp_value[0] = 0.0;
        tmp_value[1] = 0.0;
        tmp_value[2] = 0.0;
        if(ps_mod < 1E-5){
        ...
        tmp_value[0] = I*sqrt(2.0)/M_PI*pow(Z/a0,1.5);
        tmp_value[0] *= (ps[0]+afield_x)/pow(ps_mod,3)*pow(ps_mod*ps_mod + (Z/a0)*(Z/a0),-(3.0-n)/2.0);
        tmp_value[0] *= tgamma(2.0-n);
        tmp_value[0] *= ps_mod*(2-n)*cos((3-n)*atan(ps_mod*a0/Z)) 
                     - sqrt(ps_mod*ps_mod+(Z/a0)*(Z/a0))*sin((2.0-n)*atan(ps_mod*a0/Z));
        tmp_value[0] *= pow(4.0*Ip/(q0*E0),n);

        tmp_value[1] = I*sqrt(2.0)/M_PI*pow(Z/a0,1.5);
        tmp_value[1] *= (ps[1]+afield_y)/pow(ps_mod,3)*pow(ps_mod*ps_mod + (Z/a0)*(Z/a0),-(3.0-n)/2.0);
        tmp_value[1] *= tgamma(2.0-n);
        tmp_value[1] *= ps_mod*(2-n)*cos((3-n)*atan(ps_mod*a0/Z)) 
                     - sqrt(ps_mod*ps_mod+(Z/a0)*(Z/a0))*sin((2.0-n)*atan(ps_mod*a0/Z));
        tmp_value[1] *= pow(4.0*Ip/(q0*E0),n);

        tmp_value[2] = I*sqrt(2.0)/M_PI*pow(Z/a0,1.5);
        tmp_value[2] *= (ps[2]+afield_z)/pow(ps_mod,3)*pow(ps_mod*ps_mod + (Z/a0)*(Z/a0),-(3.0-n)/2.0);
        tmp_value[2] *= tgamma(2.0-n);
        tmp_value[2] *= ps_mod*(2-n)*cos((3-n)*atan(ps_mod*a0/Z)) 
                     - sqrt(ps_mod*ps_mod+(Z/a0)*(Z/a0))*sin((2.0-n)*atan(ps_mod*a0/Z));
        tmp_value[2] *= pow(4.0*Ip/(q0*E0),n);
        return tmp_value;
    }
